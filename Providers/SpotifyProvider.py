...
class SpotifyProvider:
    REQUEST_DELAY = 5.0
    _EXCLUDE_TITLE_RE = MusicPatterns.ALT_VERSION_RE
    _VERSION_TAG_RE = MusicPatterns.VERSION_TAG_RE

    def __init__(self, client_id, client_secret, logger=None, matcher=None):
    ...
    def search(self, title, artist="", album="", duration_ms=None, isrc=""):
        ...
        if best:
            self.log.debug(f"[Spotify] Trovato: '{best.get('name')}' di '{best.get('artists', [{}])[0].get('name', '')}'")
        return best

    def search_allow_version_tag(self, title, artist="", album="", duration_ms=None, isrc=""):
        if not self.is_active or not title:
            return None
        art_primary = TextCleaner.primary_artist(artist) if artist else ""
        ...
        if best:
            self.log.debug(f"[Spotify] (retry tag) Trovato: '{best.get('name')}' di '{best.get('artists', [{}])[0].get('name', '')}'")
        return best
    ...
    def _pick_best(self, tracks, title, artist, duration_ms, allow_tag):
        art_primary = TextCleaner.primary_artist(artist) if artist else ""
        ...
        for t in tracks:
            ...
            score = self.matcher.score_candidate(title=title, artist=art_primary, album_hint="", duration_ms=duration_ms, isrc="", candidate=candidate)
            if score is None:
                continue
            if score > best_score:
    ...
    def map_to_meta(track):
        ...
        album_artist = (album_artists[0].get("name", "") if album_artists else artists[0].get("name", "") if artists else "")
        ...
        return {
            "title": track.get("name", ""), "artist": artist_name, "artists_list": all_artists, "artist_ids": artist_ids,
            "album_artist": album_artist, "album_artists_list": all_album_artists, "album": album_obj.get("name", ""),
            "album_id": album_obj.get("id", ""), "album_type": album_obj.get("album_type", ""), "year": year,
            "track_number": track.get("track_number", 0), "disc_number": track.get("disc_number", 0),
            "total_tracks": album_obj.get("total_tracks", 0), "cover_url": cover_url, "explicit": bool(track.get("explicit", False)),
            "isrc": isrc, "duration_ms": track.get("duration_ms"), "artist_collection": artist_collection,
        }

    def spotify_track_to_candidate(self, track):
        artists = track.get("artists", [])
        album_obj = track.get("album", {}) or {}
        external_ids = track.get("external_ids", {}) or {}
        return {
            "trackName": track.get("name", "") or "", "artistName": artists[0].get("name", "") if artists else "",
            "collectionName": album_obj.get("name", "") or "", "trackTimeMillis": track.get("duration_ms"),
